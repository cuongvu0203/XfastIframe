    ...
    var e, t, n = {
            ...
            1569: (e, t, n) => {
                ...
                const o = function(e) {
                    ...
                    s = function() {
                        const e = "//ssl.p.jwpcdn.com/player/v/8.26.8";
                    ...
                    v = e => {
                        const t = "file:" === window.location.protocol ? "https:" : "",
                            n = {
                                ...
                                jwpsrv: "//ssl.p.jwpcdn.com/player/v/8.26.8/jwpsrv.js",
...
jwplayer.key = "F1U56v9F9VYRtROmtMj5qrBBF9G+XntIlkVwPw==";
jQuery(function($) {
    $(document).ready(function() {
        function createSmallPlayer() {
            player = jwplayer("mini-player");
            player.setup({
                file: 'http://playertest.longtailvideo.com/adaptive/wowzaid3/playlist.m3u8',
                autostart: true,
                mute: true,
                repeat: true,
                controls: false,
        ...
        $('#trv-close').on('click', function() {
            $('.mini-player').hide();
        });

        function initPlayer(mv_id, cache, preview, ads) {
            $.ajax({
                url: 'https://localhost:44336/api/Xfast',
                type: 'POST',
                contentType: 'application/json', // Đặt loại dữ liệu là JSON
                data: JSON.stringify({ // Chuyển đổi dữ liệu thành chuỗi JSON
                    mv_id: mv_id,
                    action: 'imdb_source',
                    cache: cache
                }),
                success: function(data) {
                    let source = data; // Đảm bảo dữ liệu trả về là JSON
                    const handleError = () => {
                        if (cache) {
                            initPlayer(mv_id, false, preview, ads);
                        }
                    };

                    if (source.type) {
                        if (source.type === 'direct') {
                            const imdbx = jwplayer('imdb-play').setup({
                                sources: JSON.parse(source.data),
                                width: "100%",
                                height: $("#imdb-play").width() * 8 / 16,
                                image: preview,
                            });
                            console.log(JSON.parse(source.data));
                            imdbx.on('error', handleError);
                        } else if (source.type === 'embed') {
                            debugger;
                            let smallPlayerInitialized = false;
                            const imdbx = jwplayer('imdb-play').setup({
                                sources: [{
                                    "file": "https://localhost/i9.mp4",
                                    "type": "mp4",
                                    "label": "360p"
                                }],
                                width: "100%",
                            ...
                            imdbx.on('play', function() {
                                debugger;
                                if (!smallPlayerInitialized) {
                                    console.log("player play");
                                    createSmallPlayer();
                                    smallPlayerInitialized = true;
                                    $('#trv-close').hide();
                                }
                            });
                            imdbx.on('error', function() {
                                debugger;
                                console.log("player error");
                                var sandbox = "";
                                imdbx.remove();
                                $.ajax({
                                    url: 'https://xfast.sbs/asset/ping.json',
                                    type: 'GET',
                                    dataType: 'text',
                                    success: function(data) {
                                        console.log(source.url);
                                        $('#imdb-play').html('<iframe src="' + source.url + '" ' + sandbox + ' width="100%" height="' + ($("#imdb-play").width() * 8 / 16) + '" allowfullscreen frameborder="0" scrolling="no"/>');
                                    },
                                    error: function() {
                                        console.log(source.url.replace('xfast.sbs', 'xfast.fun'));
                                        $('#imdb-play').html('<iframe src="' + source.url.replace('xfast.sbs', 'xfast.fun') + '" ' + sandbox + ' width="100%" height="' + ($("#imdb-play").width() * 8 / 16) + '" allowfullscreen frameborder="0" scrolling="no"/>');
                                    }
                                });
                                if (!smallPlayerInitialized) {
                                    createSmallPlayer();
